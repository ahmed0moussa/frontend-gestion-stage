...
import {
  addinstructorgridDataSuccess,
  addinstructorlistDataSuccess,
  deleteinstructorgriduccess,
  deleteinstructorlistSuccess,
  fetchinstructorListData,
  fetchinstructorListFailure,
  fetchinstructorListSuccess,
  fetchinstructorgridData,
  fetchinstructorgridFailure,
  fetchinstructorgriduccess,
  updateinstructorgridDataSuccess,
...
export const InstructorReducer = createReducer(
  ...
  on(fetchinstructorgriduccess, (state, { instructorGrid }) => {
  ...
  on(addinstructorlistDataSuccess, (state, { newData }) => {
    return {
      ...state,
      instructorlist: [newData, ...state.instructorlist],
      error: null,
    };
  }),
  on(updateinstructorlistDataSuccess, (state, { updatedData }) => {
    return {
      ...state,
      instructorlist: state.instructorlist.map((instructorlist) =>
        instructorlist.id === updatedData.id ? updatedData : instructorlist
      ),
      error: null,
    };
  }),
  ...
  on(deleteinstructorlistSuccess, (state, { id }) => {
    const updatedlist = state.instructorlist.filter(
      (instructorlist) => !id.includes(instructorlist.id)
    );
  ...
  on(addinstructorgridDataSuccess, (state, { newData }) => {
    return {
      ...state,
      instructorGrid: [newData, ...state.instructorGrid],
      error: null,
    };
  }),
  on(updateinstructorgridDataSuccess, (state, { updatedData }) => {
    return {
      ...state,
      instructorGrid: state.instructorGrid.map((instructorGrid) =>
        instructorGrid.id === updatedData.id ? updatedData : instructorGrid
      ),
      error: null,
    };
  }),

  on(deleteinstructorgriduccess, (state, { id }) => {
    const updatedlist = state.instructorGrid.filter(
      (instructorGrid) => !id.includes(instructorGrid.id)
    );
