...
import {
  addlistingGridData,
  addlistingGridDataFailure,
  addlistingGridDataSuccess,
  deletelistingGridData,
  deletelistingGridFailure,
  deletelistingGriduccess,
  fetchRealestateGridData,
  fetchRealestateGridFailure,
  fetchRealestateGriduccess,
  fetchearningdataData,
  fetchearningdataFailure,
  fetchearningdataSuccess,
  fetchlistingGridData,
  fetchlistingGridFailure,
  fetchlistingGriduccess,
  fetchlistinglistData,
  fetchlistinglistDataFailure,
  fetchlistinglistDataSuccess,
  updatelistingGridData,
  updatelistingGridDataFailure,
  updatelistingGridDataSuccess,
} from './apprealestate.action';
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.CrudService.fetchData('/app/listingGrid').pipe(
          map((listingGridData) => fetchlistingGriduccess({ listingGridData })),

          catchError((error) => of(fetchlistingGridFailure({ error })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.CrudService.fetchData('/app/listinglistcard').pipe(
          map((RealestateGridData) =>
            fetchRealestateGriduccess({ RealestateGridData })
          ),

          catchError((error) => of(fetchRealestateGridFailure({ error })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.CrudService.fetchData('/app/earningdata').pipe(
          ...
          catchError((error) => of(fetchearningdataFailure({ error })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.CrudService.fetchData('/app/listingList').pipe(
          map((listinglistData) =>
            fetchlistinglistDataSuccess({ listinglistData })
          ),

          catchError((error) => of(fetchlistinglistDataFailure({ error })))
        )
      )
    )
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(({ id }) =>
        this.CrudService.deleteData('/app/listingGrid').pipe(
          map(() => deletelistingGriduccess({ id })),
