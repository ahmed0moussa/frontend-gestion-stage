...
export const AppRealestateReducer = createReducer(
  ...
  on(fetchlistingGriduccess, (state, { listingGridData }) => {
  ...
  on(addlistingGridDataSuccess, (state, { newData }) => {
    return {
      ...state,
      listingGridData: [newData, ...state.listingGridData],
      error: null,
    };
  }),
  on(updatelistingGridDataSuccess, (state, { updatedData }) => {
    return {
      ...state,
      listingGridData: state.listingGridData.map((listingGridData) =>
        listingGridData.id === updatedData.id ? updatedData : listingGridData
      ),
      error: null,
    };
  }),

  on(deletelistingGriduccess, (state, { id }) => {
    const updatedlist = state.listingGridData.filter(
      (listingGridData) => !id.includes(listingGridData.id)
    );
