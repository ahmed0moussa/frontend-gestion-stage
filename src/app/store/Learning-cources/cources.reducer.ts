...
import {
  addcourcegridDataSuccess,
  addcourcelistDataSuccess,
  deletecourcegriduccess,
  deletecourcelistSuccess,
  fetchcourcegridFailure,
  fetchcourcegriduccess,
  fetchcourcegriddata,
  fetchcourcelistFailure,
  fetchcourcelistSuccess,
  fetchcourcelistdata,
  updatecourcegridDataSuccess,
  updatecourcelistDataSuccess,
} from './cources.action';
...
export const CourcesReducer = createReducer(
  ...
  on(addcourcelistDataSuccess, (state, { newData }) => {
    return {
      ...state,
      courcelistdata: [newData, ...state.courcelistdata],
      error: null,
    };
  }),
  on(updatecourcelistDataSuccess, (state, { updatedData }) => {
    return {
      ...state,
      courcelistdata: state.courcelistdata.map((courcelistdata) =>
        courcelistdata.id === updatedData.id ? updatedData : courcelistdata
      ),
      error: null,
    };
  }),
  ...
  on(deletecourcelistSuccess, (state, { id }) => {
    const updatedlist = state.courcelistdata.filter(
      (courcelistdata) => !id.includes(courcelistdata.id)
    );
  ...
  on(fetchcourcegriduccess, (state, { Griddata }) => {
  ...
  on(updatecourcegridDataSuccess, (state, { updatedData }) => {
    return {
      ...state,
      Griddata: state.Griddata.map((Griddata) =>
        Griddata.id === updatedData.id ? updatedData : Griddata
      ),
      error: null,
    };
  }),

  on(deletecourcegriduccess, (state, { id }) => {
